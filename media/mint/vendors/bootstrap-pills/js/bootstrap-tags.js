(function($) {
    ...
    function Tags(context, params) {
        ...
        var pills_list = $self.options.templates.list.appendTo(context);
        ...
        if($self.options.can_add) {
            ...
            if($self.options.suggestion_url) {
                $.ajax({
                    ...
                    data: {limit: $self.options.suggestion_limit}
                }).done(function(json) {
                        if(typeof json.result == "object") {
                            suggestions = $.merge(suggestions, json.result);
                        }
                    });
            }
    ...
    Tags.prototype.addTag = function(pills_list, value) {
        ...
        if($self.options.can_delete) {
            icon = $(document.createElement('a'))
                ...
                .html($self.options.templates.delete_icon)
                .click(function() {
                    $self.removeTag(this);
                });
        }
