...
?>
    ...
    <div id="flow-files-container-<?php echo $field->id; ?>" class="position-relative">
        <div id="flow-files<?php echo $field->id; ?>" class="flow-files mb-3">
			<?php if ($field->params->get('params.flow_drop', 1)): ?>
                <div class="flow-files-item flow-drop me-2 text-center" ondragenter="jQuery(this).addClass('flow-dragover');"
			...
			<?php if ($field->params->get('params.flow_file', 1)): ?>
                <button type="button" class="btn btn-light border flow-files-item flow-browse"><i
                            class="fas fa-file"></i> <?php echo Mint::_('CUP_SELECT_FILE') ?></button>
			<?php endif; ?>
			<?php if ($field->params->get('params.flow_folder', 1) && !$field->iscomment): ?>
                <button type="button" class="btn btn-light border  flow-browse-folder ms-2"><i
                            class="fas fa-folder"></i> <?php echo Mint::_('CUP_SELECT_FOLDER') ?></button>
			<?php endif; ?>
			<?php if ($field->params->get('params.flow_img', 1) && !$field->iscomment): ?>
                <button type="button" class="btn btn-light border  flow-files-item flow-browse-image ms-2"><i
                            class="fas fa-image"></i> <?php echo Mint::_('CUP_SELECT_IMAGE') ?></button>
			<?php endif; ?>
        </div>
        ...
        <div class="table-responsive card overflow-hidden">
            <table class="table table-stripped  m-0" id="flow-list<?php echo $field->id; ?>">
                <thead>
                <tr>
                    <th>Name</th>
                    <th width="1%">Size</th>
                                    ...
                                    <div class="float-end" id="flow-menu<?php echo $field->id; ?>">
										<?php if ($can_delete): ?>
                                            <a href="javascript:void(0);" class="btn-delete text-danger"
                                               data-id="<?php echo $f['id'] ?>"><i class="fas fa-times"></i></a>
										<?php endif; ?>
										<?php if ($field->params->get('params.allow_edit_title') || $field->params->get('params.allow_add_descr')): ?>
                                            <a href="javascript:void(0);" class="btn-edit text-primary"
                                               data-id="<?php echo $f['id'] ?>"
                                               data-json='<?php echo json_encode($f) ?>'><i class="fas fa-edit"></i></a>
										<?php endif; ?>
										<?php if ($field->params->get('params.flow_default', 1)): ?>
                                            <a href="javascript:void(0);" class="hasTooltip btn-make-default"
                                               title="<?php echo \Joomla\CMS\Language\Text::_('FF_MAKEDEFAULT') ?>"
                                               data-id="<?php echo $f['id'] ?>"><?php echo HTMLFormatHelper::icon((@$f['default'] ? 'status-away' : 'status-offline') . '.png') ?></a>
										<?php endif; ?>
                                    </div>


								<?php endif; ?>
                                <span class="flow-has-descr hasTooltip<?php echo $f['description'] ? '' : ' hide' ?>"
                                      title="<?php echo strlen($f['description']) > 100 ? (substr($f['description'], 0, 100)) . ' ...' : $f['description'] ?>"><i
    ...
    <ul class="text-muted list-unstyle mt-3">
        <li>
            <small>
                ...
                <b><?php echo $field->params->get('params.file_formats'); ?></b>
            </small>
        </li>
        <li>
            <small>
                <i class="fas fa-info-circle"></i> <?php echo \Joomla\CMS\Language\Text::_('CNSG_MAXSIZEPERFILE') ?>:
                <b><?php echo $field->params->get('params.max_size'); ?> MB</b>
            </small>
        </li>
    </ul>
...
<?php if (($field->params->get('params.allow_edit_title') || $field->params->get('params.allow_add_descr')) && !$field->iscomment): ?>
    <div id="myModal<?php echo $field->id; ?>" class="modal fade" tabindex="-1" role="dialog"
         aria-labelledby="myModalLabel<?php echo $field->id; ?>" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header py-2">
                    <h4 class="m-0"
                        id="myModalLabel<?php echo $field->id; ?>"><?php echo \Joomla\CMS\Language\Text::_('CUP_EDIT_FILE') ?></h4>
                ...
                <div class="modal-body">
                    ...
                    <div class="<?php echo($field->params->get('params.allow_add_descr') ? '' : 'hide') ?>">
                        ...
                        <textarea class="form-control" name="description" id="description<?php echo $field->id; ?>"
                                  cols="30" rows="10"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button id="closeUploadModal" type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal"><?php echo Mint::_("CCLOSE") ?></button>
