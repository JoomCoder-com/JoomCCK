...
class JoomcckModelRecords extends MModelList
{
	...
	public function populateState($ordering = NULL, $direction = NULL)
	{
		...
		$type = \Joomla\Utilities\ArrayHelper::toInteger($type);
		...
		$user = \Joomla\Utilities\ArrayHelper::toInteger($user);
		...
		$tag = \Joomla\Utilities\ArrayHelper::toInteger($tag);
		...
		$cat = \Joomla\Utilities\ArrayHelper::toInteger($cat);
	...
	public function where($query)
	{
		...
		{
			$this->_ids = \Joomla\Utilities\ArrayHelper::toInteger($this->_ids);
			$query->where('r.id IN(' . implode(',', $this->_ids) . ')');
		}

		if(!empty($this->_id_limit) && is_array($this->_id_limit))
		{
			$this->_id_limit = \Joomla\Utilities\ArrayHelper::toInteger($this->_id_limit);
		...
		if($view_what == 'compare')
		{
			...
			$list = \Joomla\Utilities\ArrayHelper::toInteger($list);
			$list[] = 0;
	...
	public function getFilterTypes()
	{
		...
		$cache[$this->section->id] = \Joomla\Utilities\ArrayHelper::toInteger($cache[$this->section->id]);
