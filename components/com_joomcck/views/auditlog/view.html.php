...
class JoomcckViewAuditlog extends MViewBase
{
	...
	function display($tpl = NULL)
	{
		...
		if(count($sections) > 0)
		{
			...
			{
				$this->sections[] = \Joomla\CMS\HTML\HTMLHelper::_('select.option', $name->id, $name->name . ' <span class="badge text-bg-light shadow-sm px-2 py-1">' . $name->total . '</span>');
		...
		if(count($types) > 0)
		{
			...
			{
				$this->types[] = \Joomla\CMS\HTML\HTMLHelper::_('select.option', $name->id, $name->name . ' <span class="badge text-bg-light shadow-sm px-2 py-1">' . $name->total . '</span>');
			}
		}
