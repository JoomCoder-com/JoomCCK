...
<?php if(JFactory::getApplication()->input->get('view_what') == 'events'):?>
	...
	<button class="btn" type="button"
		onclick="window.location = '<?php echo JRoute::_('index.php?option=com_joomcck&view=notifications&return='.Url::back());?>'">
	...
	<button class="btn" type="button" onclick="window.location = '<?php echo JRoute::_('index.php?option=com_joomcck&view=options&return='.Url::back())?>'">
					...
					<?php if($params->get('personal.user_message')):?>
						<div class="lead">
							...
							<?php elseif($this->isMe && !CUsrHelper::getOption($user, "sections.{$this->section->id}.description") && $this->section->params->get('personalize.personalize', 1)):?>
								<?php echo JText::sprintf('CEDITWELCOMETEXT', JRoute::_('index.php?option=com_joomcck&view=options&layout=section&section_id='.$this->section->id.'&return='.Url::back()))?>
							<?php endif;?>
						</div>
					<?php endif;?>
				</td>
						...
						<?php if($category): ?>
							<?php if($params->get('personal.user_categories_icons')):?>
								<div class="cat-icon" style="background-image: url(<?php
									if($category->icon) echo CImgHelper::getThumb(JPATH_ROOT. '/images/usercategories'. DIRECTORY_SEPARATOR .JFactory::getApplication()->input->getInt('user_id'). DIRECTORY_SEPARATOR .$category->icon,
