...
<table class="table table-hover" id="articleList">
	...
	<tr>
		...
		<th width="1%">
			<!--<input type="checkbox" id="checkMain" name="toggle" value=""  onclick="checkAll(<?php echo count($this->items->articleform); ?>);" />-->
		</th>
		<th class="title">
	...
	foreach($this->items->articleform AS $i => $item)
	{
		$k     = 1 - $k;
		$ident = '[' . $item->ident . '],[' . $item->type . ']';
		$link  = 'index.php?option=com_joomcck&view=templates&layout=form&cid[]=' . $ident . '&tab=param-page5';
