...
foreach($this->statuses as $key => $status)
{
	if ($this->checkStatus($params->get('params.access' . $key, $access[$key])))
		$options[] = \Joomla\CMS\HTML\HTMLHelper::_('select.option', $key, $status);
}
?>

<?php if (count($options)) :?>
	<?php echo \Joomla\CMS\HTML\HTMLHelper::_('select.genericlist', $options, 'jform[fields][' . $this->id . ']', $class . $required . $style, 'value', 'text', $this->default, 'field_' . $this->id);?>
<?php endif; ?>
