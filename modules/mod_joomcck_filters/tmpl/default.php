		...
		<div class="mb-3 typesFilter">
            ...
            <div class="<?php echo($state->get('records.type') ? ' active' : NULL) ?>">
				<?php if($params->get('filter_type_type') == 1): ?>
					...
					<?php endforeach; ?>
				<?php else : ?>
					<?php echo JHtml::_('select.genericlist', $f_types, 'filters[type]', "class='form-select' ", 'id', 'name', $state->get('records.type')); ?>
				<?php endif; ?>
            </div>
        </div>
	<?php endif; ?>
			...
			<div class="mb-3">
                <label class="form-label">
					...
					<?php if($filter->params->get('params.filter_descr')): ?>
                        <small rel="tooltip" title="<?php echo JText::_($filter->params->get('params.filter_descr')); ?>"><i class="fas fa-info-circle text-muted"></i></small>
					<?php endif; ?>
                </label>
