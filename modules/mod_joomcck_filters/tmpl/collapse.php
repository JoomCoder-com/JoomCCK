...
<form action="<?php echo \Joomla\CMS\Router\Route::_('index.php'); ?>" method="post" name="filterform" id="filter-form">
	<?php if ($params->get('filter_search', 1)): ?>
        <div class="<?php echo($state->get('records.search') ? ' active' : null) ?>">
            <input type="text" placeholder="<?php echo \Joomla\CMS\Language\Text::_('CSEARCHPLACEHOLDER') ?>"
                   class="form-control" name="filter_search" value="<?php echo $state->get('records.search'); ?>"/>
        </div>
	<?php endif; ?>

	<?php if (count($f_types) > 1): ?>
        <div data-bs-toggle="collapse" data-bs-target="#filter-types"
             class="filter-label <?php echo(!!$state->get('records.type') ? null : 'collapsed') ?>">
			...
			<?php if ($params->get('filter_type_type') == 1): ?>
				<?php foreach ($f_types as $type): ?>
                    <label class="checkbox">
                        <input id="type-<?php echo $type->id ?>" type="checkbox" name="filters[type][]"
                               value="<?php echo $type->id ?>"<?php echo $type->filter_checked ?>>
						<?php echo $type->name; ?>
                    </label>
				<?php endforeach; ?>
			<?php else : ?>
	...
	<?php if ($params->get('filter_tags_type')): ?>
        <div data-bs-toggle="collapse" data-bs-target="#filter-tags"
             class="filter-label <?php echo(!!$state->get('records.tag') ? null : 'collapsed') ?>">
			<?php if ($params->get('show_icons', 1)): ?>
	...
	<?php if ($params->get('filter_users_type')): ?>
        <div data-bs-toggle="collapse" data-bs-target="#filter-users"
             class="filter-label <?php echo(!!$state->get('records.user') ? null : 'collapsed') ?>">
			<?php if ($params->get('show_icons', 1)): ?>
	...
	<?php if ($params->get('filter_category_type')): ?>
        <div data-bs-toggle="collapse" data-bs-target="#filter-cats"
             class="filter-label <?php echo(!!$state->get('records.category') ? null : 'collapsed') ?>">
			<?php if ($params->get('show_icons', 1)): ?>
	...
	<?php foreach ($filters as $filter): ?>
        ...
        <div data-bs-toggle="collapse" data-bs-target="#filter-<?php echo $filter->key ?>"
             class="filter-label <?php echo($filter->isFilterActive() ? null : 'collapsed') ?>">
			...
			<?php if ($filter->params->get('params.filter_descr')): ?>
                <small rel="tooltip"
                       data-bs-original-title="<?php echo \Joomla\CMS\Language\Text::_($filter->params->get('params.filter_descr')); ?>"><i
                            class="icon-help"></i></small>
			<?php endif; ?>
        </div>
        <div id="filter-<?php echo $filter->key ?>"
             class="<?php echo($filter->isFilterActive() ? 'in' : null) ?> collapse">
    ...
    <div class="form-actions mt-3">
	    ...
	    <?php if ($params->get('show_reset_button', 1)): ?>
            <button onclick="Joomla.submitbutton('records.cleanall');" class="btn btn-outline-danger">
                <i class="fas fa-eraser"></i> <?php echo Text::_('CORESET'); ?>
            </button>
	    <?php endif; ?>
    </div>
</form>
